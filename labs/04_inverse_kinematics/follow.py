...
class JacobianDemo():
    ...
    def eight(t,fx=0.3,fy=0.6,rx=.1,ry=.1):
        ...
        #own tracjectory
        # x0 = np.array([0.307, 0, 0.487]) # corresponds to neutral position
        # xdes = x0 + np.array([0, rx*sin(fx*t),ry*sin(fy*t)])
        # vdes = np.array([0, rx*fx*cos(fx*t),ry*fy*cos(fy*t)])
    ...
    def ellipse(t,f=0.5,ry=.15,rz=.10):
        ...
        #own tracjectory
        # xdes = JacobianDemo.x0+np.array([ry-ry*cos(f*t), 0, rz*sin(f*t)])
